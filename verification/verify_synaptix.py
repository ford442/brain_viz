...
def verify() -> None:
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            page.wait_for_function("() => Boolean(window.__synaptixDebug)", timeout=15000)
            page.wait_for_timeout(2500)

            overlay_visible = page.locator("#error.visible").count() > 0
            if overlay_visible:
                raise AssertionError("WebGPU error overlay became visible")
            ...
            if errors:
                ...
                hard_errors = [
                    entry for entry in hard_errors
                    if "WebGPU Context is Lost" not in entry and "GPUDeviceLostInfo" not in entry
                ]
                if hard_errors:
