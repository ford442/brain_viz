...
- **`shaders.js`** — Thin barrel re-exporting the WGSL sources in `src/shaders/*.js` (one module per pipeline: mesh, fiber, soma-render, spark-render, pointcloud-render, post-fx, volumetric-compute). Shared WGSL helpers live in `src/shaders/render-shared.js`; the uniform structs are generated by `src/shaders/uniform-layout.js`.
...
- Add one entry to the layout array. That is the whole change.
- Never hand-write `struct Uniforms { … }` in a shader, an `OFFSET_*` number, a `padN` filler field, or a buffer byte count — `npm test` fails the build if you do
- Misalignment causes silent data corruption, which is why this is generated
...
1. Add one entry to `RENDER_UNIFORM_LAYOUT` (or `COMPUTE_UNIFORM_LAYOUT`) in `src/shaders/uniform-layout.js`.
2. Read it in WGSL as `uniforms.<name>` / `params.<name>`; the struct and padding are generated.
3. Write it in `src/brain-renderer/uniforms.js` via `R.<name>` (render) or `cOff('<name>')` (compute).
4. Add a UI slider to `index.html` and wire it in `main.js`, then run `npm test`.
